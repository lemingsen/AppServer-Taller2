...
import appserver.services.user_services
...
def test_register_if_user_exists_returns_409_status(register_mock, client, user_data):
    ...
    response = client.post('/user/register',
                           json=user_data.valid_user)
    assert response.status_code == 409


def test_register_if_invalid_user_returns_400_status(client, user_data):
    response = client.post('/user/register',
                           json=user_data.invalid_user)
    assert response.status_code == 400


@patch.object(appserver.services.user_services.UserService, 'get_profile')
def test_get_profile_if_user_not_exists_returns_404_status(get_profile_mock, client, user_data):
    ...
    response = client.get('/user/profile',
                          headers=user_data.valid_token_header())
...
def test_modify_profile_if_body_not_json_returns_400_status(client, user_data):
    response = client.put('/user/profile',
                          headers=user_data.valid_token_header(),
                          data=user_data.not_json(),
